...
export const createJob = async (data, userToken) => {
  ...
  if (user.user_status !== 'Active') {
    throw new CustomError('Usuário não autorizado a criar vagas', 403);
  }
...
export const getJobs = async (userToken, page = 1) => {
  ...
  if (user.user_status !== 'Active') {
    throw new CustomError('Usuário não autorizado a acessar as vagas', 403);
  }
...
export const getJobById = async (userToken, jobId) => {
  ...
  if (user.user_status !== 'Active') {
    throw new CustomError('Usuário não autorizado a acessar a vaga', 403);
  }
...
export const updateJob = async (jobId, data, userToken) => {
  ...
  if (user.user_status !== 'Active') {
    throw new CustomError('Usuário não autorizado a editar vagas', 403);
  }
...
export const deleteJob = async (jobId, userToken) => {
  ...
  if (user.user_status !== 'Active') {
    throw new CustomError('Usuário não autorizado a excluir vagas', 403);
  }
